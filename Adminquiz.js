...
function Adminquiz() {
  ...
  const filesListRef = ref(storage, `courses/${id}/quiz/`);

  const uploadFile = () => {
    if (fileUpload == null) return;
    const fileRef = ref(storage, `courses/${id}/quiz/${fileUpload.name + v4()}`);
  ...
  const uploadTextFile = () => {
    ...
    const textFileRef = ref(storage, `courses/${id}/quiz/${fileName}`);
