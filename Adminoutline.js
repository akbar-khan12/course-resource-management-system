...
function Adminoutline() {
  ...
  const filesListRef = ref(storage, `courses/${id}/outline/`);

  const uploadFile = () => {
    if (fileUpload == null) return;
    const fileRef = ref(storage, `courses/${id}/outline/${fileUpload.name + v4()}`);
  ...
  const uploadTextFile = () => {
    ...
    const textFileRef = ref(storage, `courses/${id}/outline/${fileName}`);
